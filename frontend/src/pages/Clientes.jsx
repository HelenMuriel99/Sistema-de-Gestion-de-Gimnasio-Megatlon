...
export default function Clientes() {
  ...
  // Filtrado de clientes según nombre completo o CI
  const clientesFiltrados = clientes.filter((cli) => {
    const busqueda = searchTerm.toLowerCase();
    const nombre = cli.nombreCompleto ? cli.nombreCompleto.toLowerCase() : '';
    const ci = cli.ci ? cli.ci.toString().toLowerCase() : '';
    
    return nombre.includes(busqueda) || ci.includes(busqueda);
  });
                ...
                {clientesFiltrados.length === 0 && (
                  <tr>
                    <td colSpan="5" className="px-6 py-8 text-center text-gray-500">
                      {searchTerm ? 'No se encontraron clientes que coincidan con la búsqueda.' : 'No hay clientes inscritos en esta sucursal.'}
                    </td>
                  </tr>
                )}
              </tbody>
